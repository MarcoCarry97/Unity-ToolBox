...
def single_model_solving(input,filename):
    input = input.replace(" ", ".") + "."
    file = open(filename)
    program = input+file.read()
    control = clingo.Control(arguments=["--model=1"])
...
def complete_model_solving(input,corr_file,dec_file):
    input=single_model_solving(input,corr_file)
    return single_model_solving(input,dec_file)
...
class maker_solver:
    ...
    def solve(self, room_file, corr_file, dec_file, num_levels, num_rooms, size, distance,path,rand_init):
        fileroom=open(room_file)
        program=fileroom.read()
        control = clingo.Control(arguments=["--model=" + str(num_levels)])
        control.add("base", [], program)
        control.ground([("base", [])])
        handle = control.solve(yield_=True)
        models=[]
        for model in handle:
            model_str=complete_model_solving(str(model),corr_file,dec_file)
            models+=[create_model_dict(model_str)]
